...
namespace Ga144.C.Preprocessor;

/// <summary>
/// The standalone CLI's own <see cref="ICIncludeResolver"/>: a plain filesystem search, exactly like
/// a C compiler's command line -- a quoted "#include \"name\"" first looks alongside the including
/// file, then falls back to the same "-I" search path list an angle-bracketed "#include &lt;name&gt;"
/// uses from the start. The IDE will supply its own resolver built from a C project's own include
/// directory and its library references' include directories instead of this one; neither resolver
/// depends on the other, which is exactly why <see cref="Ga144.C.Toolchain.CPreprocessor"/> only ever
/// talks to the <see cref="ICIncludeResolver"/> interface.
/// </summary>
