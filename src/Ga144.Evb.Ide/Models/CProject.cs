...
public sealed class CProject
{
  ...
  /// <summary>Hand-written CVM assembly (".asm") source files that are simply part of this project --
  ...
  /// <b>Extension changed 2026-09-09 (per Stefan): ".asm" for a hand-written file, ".casm" reserved for
  /// what the C compiler generates into <see cref="TargetAssemblyDirectoryName"/></b> -- the two used
  /// to share ".casm", which made a hand-written file and a compiler-generated one indistinguishable by
  /// extension alone.</summary>
  public const string AssemblyDirectoryName = "asm";

  /// <summary>Holds this project's own assembled object files (one ".gaobj" per assembled assembly
  /// file -- ".casm" if compiler-generated, ".asm" if hand-written, see
  /// <see cref="AssemblyDirectoryName"/>) -- entirely generated by Build, safe to delete, and
  /// overwritten every time. Added 2026-09-07 alongside
  ...
  /// <summary>Matches this project's own hand-written CVM assembly files under
  /// <see cref="AssemblyDirectoryName"/> -- CHANGED 2026-09-09 from "*.casm" to "*.asm" (see
  /// <see cref="AssemblyDirectoryName"/>'s own remarks); <see cref="TargetAssemblyDirectoryName"/>'s
  /// compiler-generated files are never matched by this pattern and are not scanned as a source list
  /// (see <see cref="GetAssemblyFiles"/>).</summary>
  public const string AssemblySearchPattern = "*.asm";
  ...
  /// <summary>Where Build should write the assembled object file for a given assembly file -- whether
  /// it came from <see cref="TargetAssemblyDirectoryPath"/> (compiler-generated, ".casm") or
  /// <see cref="AssemblyDirectoryPath"/> (hand-written, ".asm") -- named after that file itself, with
