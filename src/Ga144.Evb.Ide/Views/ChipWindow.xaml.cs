...
public partial class ChipWindow : Window
{
    ...
    private void OnChipDrawingGridSizeChanged(object sender, SizeChangedEventArgs e)
    {
        ...
        if (_viewModel.KrakenInstalled && e.NewSize.Width > 1.0 && e.NewSize.Height > 1.0)
        {
            DrawKrakenPaths();
        }
    }
    ...
    {
        // The Kraken arrows depend only on the project-side topology. Runtime
        // status changes (ONLINE, endpoint text, etc.) must not rebuild the
        // large 2448x1152 visual tree. The TextBlock binding updates runtime
