...
/// that node, the register/stack initialization to apply once loading is complete, and the
...
/// <c>/IO</c>, <c>/STACK</c>, <c>/RSTACK</c>, <c>/P</c> -- since that is exactly what this record
/// holds, just produced directly from this project's own <see cref="F18Compiler"/> instead of
/// hand-authored Boot Descriptor Language. Every value here is either what the node's own
/// source's <c>/a</c>/<c>/b</c>/<c>/io</c>/<c>/stack</c>/<c>/rstack</c>/<c>entry</c> directives
/// specified, or the documented reset default (see <see cref="F18CompileResult"/>'s own remarks)
/// when a directive was not used.
///
...
public sealed record CvmBootDescriptor(
    ...
    IReadOnlyList<int> InitialReturnStack,
    int? EntryPoint)
{
  /// <summary>
