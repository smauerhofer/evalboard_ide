...
A textual macro is expanded with the prefix keyword `macro` followed by the macro name. `macro` is a defining-style word (like `:`) whose argument is the macro to expand in place:

```forth
macro rom_relay
```

System macros may be expanded in ROM or RAM; project user macros are RAM-only. (The former postfix form `rom_relay import` is no longer accepted; the compiler emits F18P007 pointing to the new `macro <name>` spelling.)
